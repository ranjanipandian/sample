...
def search_with_embeddings(query, search_types):
    ...
    try:
        ...
        if "Semantic" in search_types:
            # Use semantic search with adaptive threshold (always returns results)
...
def main():
            ...
            with progress_container:
                ...
                if st.session_state.search_mode == "LLM":
                    ...
                    try:
                        ...
                        # Get answer from GPT-4o (returns dict with 'answer' and 'insights')
                        ...
                        # Display answer and insights
                        st.markdown("---")
                        st.markdown('<h3 style="font-size: 0.95rem;">💬 LLM Q&A Response</h3>', unsafe_allow_html=True)
                        
                        # Display the main answer
                        answer_text = result.get('answer', result) if isinstance(result, dict) else result
                        st.markdown(f'<div style="background: #f0f8ff; padding: 1rem; border-radius: 10px; border-left: 4px solid #6fa8dc;"><p style="font-size: 0.85rem; color: #000; line-height: 1.6;">{answer_text}</p></div>', unsafe_allow_html=True)
                        
                        # Display insights if available
                        if isinstance(result, dict) and result.get('insights'):
                            st.markdown("---")
                            st.markdown('<h4 style="font-size: 0.9rem;">💡 Key Insights</h4>', unsafe_allow_html=True)
                            for i, insight in enumerate(result['insights'], 1):
                                st.markdown(f'<div style="background: #d4edda; padding: 0.8rem; border-radius: 8px; margin: 0.5rem 0; border-left: 3px solid #00cc66;"><p style="font-size: 0.8rem; color: #000; margin: 0;"><strong>{i}.</strong> {insight}</p></div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander("💬 Available Models", expanded=False):
            st.markdown('<p style="font-size: 0.65rem; margin: 0.1rem 0;">🔷 <strong>GPT-4o Mini</strong></p>', unsafe_allow_html=True)
            st.markdown('<p style="font-size: 0.65rem; margin: 0.1rem 0;">🔶 <strong>Claude Sonnet 4.5</strong></p>', unsafe_allow_html=True)
